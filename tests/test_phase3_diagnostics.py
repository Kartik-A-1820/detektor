...
class TestPhase3Diagnostics(unittest.TestCase):
    ...
    def test_reports_effective_matches_for_sub_stride_ball_boxes(self) -> None:
        ...
        # 0.01 * 512 ~= 5.12 px wide/high, which is smaller than the stride-8 grid spacing.
        ...
        self.assertEqual(diagnostics["train"]["classes"]["ball"]["zero_assignment_gt"], 0)
        self.assertEqual(diagnostics["val"]["classes"]["ball"]["zero_assignment_gt"], 0)
        self.assertGreaterEqual(diagnostics["train"]["classes"]["ball"]["median_assigned_points"], 1.0)
        self.assertEqual(diagnostics["assigner"]["min_effective_box_size"], 8.0)
